...
class OPSHandler(Handler):
    ...
    def handle(self,obj,f):
        if f and f.name.endswith('.xls'):
            ...
            for sheet in workbook.sheets():
                if sheet.name == '部门' or sheet.name == 'department':
                    self.department(obj,sheet)
                elif sheet.name == '岗位' or sheet.name == 'position':
                    self.position(obj,sheet)
                elif sheet.name == '职员' or sheet.name == 'employee':
    ...
    def stuff(self,obj,sheet):
        ...
        row_count = sheet.nrows
        try:
            group = Group.objects.get_by_natural_key('职员')
        except Exception as e:
            pass
        for row_index in range(1,row_count):
            row = sheet.row_values(row_index)
            ...
            password = row[4][-6:]
            if position is None:
                raise Exception('职员%s-%s未分配岗位，或者您选择的岗位已失效，不可被引用'%(row[0],row[1]))
            try:
